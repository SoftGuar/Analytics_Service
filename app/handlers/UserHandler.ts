...
export class UserHandler {
    ...
    static async getUserSessionDuration(req: FastifyRequest, reply: FastifyReply) {
        try {
            const sessionDurations = await UserSessionsService.getUserSessionDuration();
            reply.status(200).send(sessionDurations);
        } catch (error) {
            console.error("Error in getUserSessionDuration handler:", error);
            reply.status(500).send({ error: "Failed to fetch user session durations" });
        }
    }
