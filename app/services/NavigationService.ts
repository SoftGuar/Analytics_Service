...
export class NavigationService {
    ...
    static async getSuccessfulNavigations(){
        try{
            const result = await prisma.$queryRaw<{date: Date; total_attempts: number; successful_navigations: number; success_rate: number}[]>
            `
                SELECT
                DATE_TRUNC('day', start_time) AS date,
                COUNT(*) AS total_attempts,
                SUM(CASE WHEN end_time IS NOT NULL AND rerouting_count = 0 THEN 1 ELSE 0 END) AS successful_navigations,
